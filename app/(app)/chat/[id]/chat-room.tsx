...
export function ChatRoom({
  ...
  async function onReply(values: ReplyForm) {
    try {
      ...
      const data = (await res.json()) as { assistantMessageId?: string; error?: string }
      if (!res.ok) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
          ...
          {initialMessages.map((m) => {
            ...
            return (
              <Card
                key={m.id}
                className={
                  m.role === "user" ? "ml-8 border-primary/20" : "mr-8 border-muted"
                }
              >
                <CardHeader className="py-3 pb-0">
                  <CardTitle className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    {m.role === "user" ? "You" : "Assistant"}
                    {m.role === "assistant" && m.status === "streaming" ? " · streaming" : null}
                    {m.role === "assistant" && m.status === "failed" ? " · failed" : null}
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-2 pb-4">
                  <p className="whitespace-pre-wrap text-sm leading-relaxed">{live || "…"}</p>
                </CardContent>
              </Card>
            )
          })}
      ...
      <div className="border-t border-border p-4">
        <form onSubmit={form.handleSubmit(onReply)} className="mx-auto flex max-w-3xl flex-col gap-3">
          <FieldGroup>
            <Field>
          ...
          <Button
            ...
            className="self-end"
          >
            {streamMessageId ? "Wait for reply…" : form.formState.isSubmitting ? "Sending…" : "Send"}
          </Button>
        </form>
      </div>
    </div>
